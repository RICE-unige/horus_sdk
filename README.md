...
| 3D Map Contracts | :large_orange_diamond: In progress | Point-cloud, mesh, and octomap global visualization payload wiring remains available for on-device rendering. First-class remote-render targets now support offboard GPU rendering, stereo RGB-D framing, camera-pose metadata, ROS-default delivery, and explicit experimental WebRTC selection. | Make WebRTC delivery as reliable as ROS, improve reprojection and Gaussian-splat fidelity, and expand native octomap interoperability beyond the metadata-only publish path. |
...
| Examples and Compatibility Guardrails | :large_orange_diamond: In progress | Public import/payload guardrail tests, curated no-CLI examples, a preserved legacy catalog, and legacy help/command documentation are in place. Remote-rendering examples now cover detailed mesh, point-cloud, and experimental Gaussian-splat scenes with download, preparation, inspection, and validation tools. | Add automated launch-smoke coverage for curated examples and payload snapshot coverage for every new contract family. |
...
| Gaussian Splat / DataViz | :large_orange_diamond: In progress | Experimental Gaussian Splat DataViz payloads, direct chunked-asset examples, fixture tools, PointCloud2 fallback, and C++/Rust parity remain integrated. The offboard GPU renderer adds detailed Gaussian-splat scene loading and stereo RGB-D output without sending full splat geometry to Quest. | Improve offboard camera correctness and visual stability, make the WebRTC path reliable, and validate dense assets beyond the diagnostic fixtures. |
