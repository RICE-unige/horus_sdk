...
class LiveRobotIntegration:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    "source horus_ros2_ws/install/setup.bash && ros2 topic list",
                ],
                capture_output=True,
                text=True,
                cwd=script_dir,
            ...
            print(
                f"     Stereo Camera: {'✓' if capabilities['has_stereo_camera'] else '✗'}"
            )
            print(f"     Odometry: {'✓' if capabilities['has_odometry'] else '✗'}")
            print(
                f"     IMU Sensors: {'✓' if capabilities['has_imu'] else '✗'} ({capabilities['imu_count']} units)"
            )
            print(
                f"     Velocity Control: {'✓' if capabilities['has_cmd_vel'] else '✗'}"
            )
    ...
    def create_comprehensive_dataviz(self, robot: Robot) -> DataViz:
        ...
        dataviz.add_robot_trajectory(
            robot_name=robot.name, topic=self.topic_mapping["odometry"], frame_id="odom"
        )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    "source horus_ros2_ws/install/setup.bash && ros2 topic list | grep -E '(global_plan|plan)'",
                ],
                capture_output=True,
                text=True,
                cwd=script_dir,
                ...
                for topic in nav_topics:
                    if "global" in topic:
                        dataviz.add_robot_global_path(
                            robot_name=robot.name, topic=topic, frame_id="map"
                        )
        ...
        try:
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    "source horus_ros2_ws/install/setup.bash && ros2 topic list | grep -E '^/map$'",
                ],
                capture_output=True,
                text=True,
                cwd=script_dir,
            ...
            if result.returncode == 0 and result.stdout.strip():
                dataviz.add_occupancy_grid(
                    topic="/map", frame_id="map", render_options={"opacity": 0.7}
                )
    ...
    def start_live_monitoring(self):
        ...
        try:
            while self.running:
                ...
                print(
                    f"   Visualizations: {len(self.dataviz.visualizations)} configured"
                )

                # Show robot color assignment
                robot_color = self.dataviz.color_manager.get_robot_color(
                    self.robot.name
                )
                ...
                try:
                    result = subprocess.run(
                        [
                            "bash",
                            "-c",
                            "source horus_ros2_ws/install/setup.bash && ros2 topic hz /front_3d_lidar/lidar_points --window 10",
                        ],
                        capture_output=True,
                        text=True,
                        timeout=3,
                    ...
                    if "average rate" in result.stdout:
                        ...
                        if hz_line:
                            print(
                                f"   3D LiDAR Rate: {hz_line[0].split(':')[1].strip()}"
                            )
                    else:
    ...
    def run_complete_integration(self):
        ...
        try:
            ...
            print(
                f"   Color: {self.dataviz.color_manager.get_robot_color(self.robot.name)}"
            )
