...
def main():
    ...
    print(
        f"   Robot visualizations: {len(wheeled_dataviz.get_robot_visualizations('rosbot_01'))}"
    )
    ...
    print(
        f"   Robot visualizations: {len(aerial_dataviz.get_robot_visualizations('drone_alpha'))}"
    )
    ...
    global_dataviz.add_occupancy_grid(
        topic="/map", render_options={"opacity": 0.7, "color_scheme": "grayscale"}
    )
    ...
    global_dataviz.add_navigation_path(
        topic="/global_path", render_options={"color": "green", "line_width": 4}
    )

    # Add TF tree visualization
    global_dataviz.add_tf_tree(render_options={"show_labels": True, "frame_scale": 0.3})

    print(f"   {global_dataviz}")
    print(
        f"   Global visualizations: {len(global_dataviz.get_global_visualizations())}"
    )
    ...
    print(
        f"   Point clouds: {len(master_dataviz.get_visualizations_by_type(VisualizationType.POINT_CLOUD))}"
    )
    print(
        f"   Paths: {len(master_dataviz.get_visualizations_by_type(VisualizationType.PATH))}"
    )
    ...
    for camera in aerial_cameras:
        print(
            f"     - {camera.name}: {camera.get_camera_type()} ({camera.get_resolution_str()})"
        )
