...
void BackendNode::status_callback(
  ...
  RCLCPP_DEBUG(get_logger(), "Received status from robot: %s",
               msg->robot_id.c_str());
...
void BackendNode::display_startup_info() {
  ...
  RCLCPP_INFO(get_logger(), "Available Plugins: %zu",
  ...
  if (check_unity_endpoint_status()) {
    ...
    RCLCPP_WARN(get_logger(), "Unity TCP Endpoint: Not detected on port %d",
                unity_tcp_port_);
  }
...
void BackendNode::register_robot_callback(
  ...
  if (!validate_robot_config(request->robot_config, validation_errors)) {
    ...
    RCLCPP_ERROR(get_logger(), "Robot registration failed: %s",
...
void BackendNode::unregister_robot_callback(
    ...
    std::shared_ptr<horus_interfaces::srv::UnregisterRobot::Response>
        response) {
  RCLCPP_INFO(get_logger(), "Robot unregistration request: %s",
  ...
  if (it != registered_robots_.end()) {
    ...
    RCLCPP_INFO(get_logger(), "Robot unregistered successfully: %s",
                request->robot_id.c_str());
  } else {
    response->success = false;
    response->error_message = "Robot not found";
    RCLCPP_ERROR(get_logger(), "Robot unregistration failed: %s not found",
                 request->robot_id.c_str());
  }
}
...
bool BackendNode::validate_robot_config(
  ...
  for (const auto& sensor : config.sensors) {
    ...
    if (sensor.topic.empty()) {
      errors.push_back("Sensor topic cannot be empty for sensor: " +
                       sensor.name);
      valid = false;
    }
  }
...
void BackendNode::monitor_robot_topics() {
  for (auto& pair : registered_robots_) {
    ...
    RCLCPP_DEBUG(get_logger(), "Robot %s health: %s", robot.robot_id.c_str(),
