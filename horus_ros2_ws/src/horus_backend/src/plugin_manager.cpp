...
void PluginManager::register_plugin(const std::string& name, std::unique_ptr<RobotPlugin> plugin) {
...
bool PluginManager::connect_robot(const std::string& robot_id, const std::string& robot_type) {
...
void GenericRobotPlugin::initialize(const std::string& robot_name) {
    std::cout << "Initializing generic robot plugin for: " << robot_name << std::endl;
}
...
std::string GenericRobotPlugin::get_robot_type() const {
    return "generic";
}

std::map<std::string, std::string> GenericRobotPlugin::get_topic_mappings() const {
    return {
        {"cmd_vel", "/cmd_vel"},
        {"odom", "/odom"},
        {"scan", "/scan"},
