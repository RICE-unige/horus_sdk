#!/usr/bin/env python3
"""Drone-focused fake TF runtime for teleop + 3D go-to-point + 3D waypoint validation."""
...
class DroneOpsSuiteFakeTFPublisher(OpsSuiteFakeTFPublisher):
    """Ops suite publisher specialized for aerial robots with 3D task execution."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Clamp initial altitudes so all robots start in valid flight bounds.
        for robot in self.robots:
        ...
        self.get_logger().info(
            f"Drone task mode enabled: 3D goals + 3D waypoint paths + teleop cmd_vel z "
            f"+ takeoff/land commands (altitude bounds {self.min_altitude_m:.2f}..{self.max_altitude_m:.2f}m)."
        )
        self.get_logger().info(
            f"Drone command topics: /<robot>/takeoff and /<robot>/land "
            f"(takeoff target {self.takeoff_altitude_m:.2f}m, land target {self.land_altitude_m:.2f}m)."
        )
        self.get_logger().info(
            f"Drone collision gating: robot-robot checks only within {self.collision_vertical_band_m:.2f}m altitude band."
        )
    ...
    def _world_direction_to_base_link(
        ...
        local_z = dz_world
        return DroneOpsSuiteFakeTFPublisher._normalize_direction_3d(local_x, local_y, local_z)
    ...
    def _resolve_collisions(self, dt: float):
        ...
        for i in range(count):
            for j in range(i + 1, count):
                ...
                # Keep altitude separation stable instead of forcing horizontal roots to "bounce" on near-overlap.
                if abs(dz) <= 1e-4:
    ...
    def _make_takeoff_callback(self, robot_name):
        def _callback(_msg: Empty):
            ...
            self._set_flight_command(robot_name, "taking_off", self.takeoff_altitude_m)
            self.get_logger().info(
                f"Takeoff command received for {robot_name} -> target altitude {self.takeoff_altitude_m:.2f}m."
            )
    ...
    def _make_land_callback(self, robot_name):
        def _callback(_msg: Empty):
            self._cancel_goal(robot_name, publish_status=True)
            self._cancel_waypoint_path(robot_name, reason="path_cancelled")
            self._set_flight_command(robot_name, "landing", self.land_altitude_m)
            self.get_logger().info(
                f"Land command received for {robot_name} -> target altitude {self.land_altitude_m:.2f}m."
            )

        return _callback

    def _publish_goal_status(self, robot_name: str, state: str, goal: Optional[DroneGoalState]):
    ...
    def _apply_teleop_to_robot(self, robot, dt: float):
        ...
        altitude = self._clamp_altitude(altitude + (linear_z * dt))
    ...
    def _apply_flight_command(self, robot, dt: float) -> bool:
        ...
        target_altitude = self._flight_target_altitude.get(
            robot_name,
            self.takeoff_altitude_m if mode == "taking_off" else self.land_altitude_m,
        )
    ...
    def _advance_goal(self, robot, goal: DroneGoalState, dt: float) -> bool:
        current_z = self._robot_altitudes.get(robot.name, self.height)
        dx = goal.target_x - robot.x
        dy = goal.target_y - robot.y
        dz = goal.target_z - current_z
    ...
    def _advance_waypoint(self, robot, goal: DroneWaypointGoal, dt: float) -> bool:
        current_z = self._robot_altitudes.get(robot.name, self.height)
        dx = goal.target_x - robot.x
        dy = goal.target_y - robot.y
        dz = goal.target_z - current_z
...
def build_drone_ops_parser():
    parser = build_ops_suite_parser()
    parser.description = (
        "Publish fake TF for aerial robots with 3D task execution: teleop cmd_vel, "
        "3D go-to goal_pose, and 3D waypoint_path."
    )
    parser.set_defaults(
        robot_count=3,
        robot_name="drone",
        robot_names="drone_1,drone_2,drone_3",
        robot_base_frames="drone_1:base_link,drone_2:base_link,drone_3:base_link",
        max_speed=1.6,
        max_turn_rate=1.8,
        height=0.3,
        static_camera=True,
    ...
    parser.add_argument(
        "--goal-altitude-tolerance",
        type=float,
        default=0.18,
        help="Go-to-point completion altitude tolerance in map meters.",
    )
    parser.add_argument(
        "--waypoint-altitude-tolerance",
        type=float,
        default=0.18,
        help="Waypoint completion altitude tolerance in map meters.",
    )
    parser.add_argument(
        "--max-vertical-speed",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.2,
        help="Target altitude in map meters when /<robot>/takeoff is received.",
    )
    parser.add_argument(
        "--land-altitude",
        type=float,
...
def run_from_args(args):
    ...
    node = DroneOpsSuiteFakeTFPublisher(
        ...
        goal_altitude_tolerance_m=args.goal_altitude_tolerance,
        waypoint_altitude_tolerance_m=args.waypoint_altitude_tolerance,
        max_vertical_speed_mps=args.max_vertical_speed,
        takeoff_altitude_m=args.takeoff_altitude,
        land_altitude_m=args.land_altitude,
